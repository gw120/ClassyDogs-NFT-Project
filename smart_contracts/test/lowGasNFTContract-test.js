...
describe("NftLowGasContract.sol", () => {
    ...
    describe("Before Sale (contract is paused)", () => {
        it("should not allow users to mint when contract is paused", async () => {
            ...
            await expect(
                contract.connect(whitelistedUser).whitelistMint(1, [], { value: mintCost })
            ).to.be.revertedWithCustomError(contract, 'NFT__ContractIsPaused');
            await expect(
    ...
    describe("Whitelist Sale", () => {
        ...
        it("should revert with invalid proof for not whitelisted user", async () => {
            ...
            await expect(contract.connect(user1).whitelistMint(
                1,
                merkleTree.getHexProof(keccak256(whitelistedUser.address)),
                { value: mintCost }
    ...
    describe("Presale (public sale but without nft reveal)", () => {
        ...
        it("should allow any user to mint", async () => {
            ...
            await contract.connect(randomUser).mint(
                3, { value: getAmountInWei(mintCost * 3) }
            )
        ...
        it("should revert on invalid mint amount", async () => {
            ...
            const mintAmount = await (await contract.maxMintAmountPerTx()) + 1
            // Sending an invalid mint amount
            await expect(contract.connect(randomUser).mint(
                mintAmount,
                { value: getAmountInWei(mintCost * mintAmount) },
            )).to.be.revertedWithCustomError(contract, 'NFT__InvalidMintAmount');
        });
        it("should revert on insufficient mint funds", async () => {
            const mintCost = getAmountFromWei(await contract.cost())
            ...
            await expect(contract.connect(randomUser).mint(
                1,
                { value: getAmountInWei(mintCost * 0.8) },
            )).to.be.revertedWithCustomError(contract, 'NFT__InsufficientFunds');
        });
    ...
    describe("Public Sale (nft revealed)", () => {
        ...
        before(async () => {
            ...
            await contract.connect(owner).setUriPrefix(CollectionConfig.baseMetadataURI)
        ...
        it("should allow any user to mint with revealed URI", async () => {
            ...
            expect(await contract.balanceOf(randomUser.address)).to.equal(6);
            expect(await contract.tokenURI(4)).to.equal(`${CollectionConfig.baseMetadataURI}4.json`)
        });

        it("wallet of user should contain all ids of his minted items", async () => {
            const randomUserWallet = Array.from((await contract.tokensOfOwner(randomUser.address)), x => Number(x))

            expect(randomUserWallet.length).to.equal(6);
            expect(randomUserWallet).to.have.members([2, 3, 4, 5, 6, 7]);
        });
    })
    ...
    describe("Only owner functions", () => {
        it("Only owner should be allowed to call this functions", async () => {
            ...
            await expect(contract.connect(randomUser).setRevealed(true)).to.be.revertedWith('Ownable: caller is not the owner');
            ...
            await expect(contract.connect(randomUser).setUriPrefix('ipfs://new-Nft-Uri/')).to.be.revertedWith('Ownable: caller is not the owner');
            await expect(contract.connect(randomUser).setUriSuffix('.jpeg')).to.be.revertedWith('Ownable: caller is not the owner');
            ...
            await expect(contract.connect(randomUser).setPaused(true)).to.be.revertedWith('Ownable: caller is not the owner');
